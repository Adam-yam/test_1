...
const state = {mode:'music',image:null,palette:['#dce1e6','#bdc6d0','#8997a5','#586978','#2c3b48'],start:0,raf:0,picking:false,busy:false,gif:null,url:null,loadToken:0,lifecutImages:[null,null,null,null],lifecutTokens:[0,0,0,0],lifecutTransforms:[{zoom:1,panX:0,panY:0},{zoom:1,panX:0,panY:0},{zoom:1,panX:0,panY:0},{zoom:1,panX:0,panY:0}],lifecutSelected:0};
...
function dimensions(){if(state.mode==='music')return $('size').value.split(',').map(Number);if(state.mode==='lifecut')return [1200,3600];return $('orientation').value==='portrait'?[1080,1920]:[1920,1080];}
...
function lifecutCrop(im,w,h,t){const scale=Math.max(w/im.width,h/im.height)*t.zoom,sw=w/scale,sh=h/scale;return {sx:(im.width-sw)*(t.panX+1)/2,sy:(im.height-sh)*(t.panY+1)/2,sw,sh};}
function lifecut(c,w,h){
 ...
 for(let i=0;i<4;i++){
  ...
  if(im){
   const p=lifecutCrop(im,cellW,cellH,state.lifecutTransforms[i]);
   c.save();c.beginPath();c.roundRect(m,y,cellW,cellH,w*.014);c.clip();c.drawImage(im,p.sx,p.sy,p.sw,p.sh,m,y,cellW,cellH);c.restore();
  }else{
...
function vignette(c,w,h,s){const g=c.createRadialGradient(w/2,h*.4,Math.min(w,h)*.15,w/2,h*.5,Math.max(w,h)*.75);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,`rgba(0,0,0,${s})`);c.fillStyle=g;c.fillRect(0,0,w,h);}
function bracket(c,x,y,w,h,len,color,lw){c.save();c.strokeStyle=color;c.lineWidth=lw;[[x,y,1,1],[x+w,y,-1,1],[x,y+h,1,-1],[x+w,y+h,-1,-1]].forEach(([cx,cy,dx,dy])=>{c.beginPath();c.moveTo(cx,cy+len*dy);c.lineTo(cx,cy);c.lineTo(cx+len*dx,cy);c.stroke();});c.restore();}
function histogram(c,x,y,w,h,color){const rnd=mulberry32(11);c.save();c.globalAlpha=.6;c.fillStyle=color;const n=22;for(let i=0;i<n;i++){const bw=w/n,bh=h*Math.max(.08,Math.sin(i/n*Math.PI)*(.6+rnd()*.4));c.fillRect(x+i*bw,y+h-bh,bw*.72,bh);}c.restore();}
function osdBox(c,x,y,w,h,r){c.save();c.fillStyle='rgba(0,0,0,.4)';c.beginPath();c.roundRect(x,y,w,h,r===undefined?h*.3:r);c.fill();c.restore();}
function pillBadge(c,x,y,w,h,fill){c.save();c.fillStyle=fill;c.beginPath();c.roundRect(x,y,w,h,h/2);c.fill();c.restore();}
function modeRow(c,cx,y,items,active,color,size){c.save();const gap=size*4.6;items.forEach((label,i)=>text(c,label,cx+(i-active)*gap,y,size,i===active?color:'#ffffff65',i===active?700:500,'center'));c.restore();}
function focus(c,w,h,time){if(['clean','grid','pro','vhs'].includes($('camera').value))return deviceCamera(c,w,h,time);const t=clamp(time,0,DURATION),kind=$('camera').value,isVHS=kind==='vhs',isParallax=kind==='parallax',isLightstick=kind==='lightstick',isPro=kind==='pro',isGrid=kind==='grid',base=Math.min(w,h),m=base*.055;const seek=clamp((t-650)/1300,0,1),e=seek*seek*(3-2*seek),locked=t>=1950;const blur=(1-e)*base*.012,shake=t<1950?Math.sin(t/91)*base*.002*(1-e):0;const accent=isPro?'#ff5252':isVHS?'#ff2d55':isParallax?'#c9a6ff':isGrid?'#3ddc84':isLightstick?'#ff8bd1':'#ffd60a';const zProg=clamp(t/DURATION,0,1),zEase=zProg*zProg*(3-2*zProg);
 if(isParallax)parallaxBG(c,w,h,t);else{c.fillStyle=isVHS?'#0a0a0a':'#101214';c.fillRect(0,0,w,h);}if(!isParallax)vignette(c,w,h,isVHS?.5:.32);const top=base*.115,bottom=base*.19,area={x:m,y:top,w:w-2*m,h:h-top-bottom};if(isVHS){c.fillStyle='#050505';c.fillRect(0,0,w,top);c.fillRect(0,h-bottom,w,bottom);c.strokeStyle='#000';c.lineWidth=base*.004;c.beginPath();c.moveTo(0,top);c.lineTo(w,top);c.moveTo(0,h-bottom);c.lineTo(w,h-bottom);c.stroke();}const jx=isVHS?Math.sin(t/45)*base*.0015+(t%171<8?base*.012:0):0;const pr=photo(c,area.x+jx,area.y,area.w,area.h,{blur,shake,radius:kind==='clean'?base*.025:0,extraZoom:isParallax?1+zEase*.08:1});
 c.save();c.strokeStyle='rgba(255,255,255,.12)';c.lineWidth=Math.max(1,base*.0018);c.beginPath();c.roundRect(area.x,area.y,area.w,area.h,kind==='clean'?base*.025:0);c.stroke();c.restore();
 ...
 if(isPro){c.save();c.strokeStyle='#ffffff22';for(let i=1;i<3;i++){line(c,area.x+area.w*i/3,area.y,area.x+area.w*i/3,area.y+area.h,1);line(c,area.x,area.y+area.h*i/3,area.x+area.w,area.y+area.h*i/3,1);}c.restore();bracket(c,area.x+base*.018,area.y+base*.018,area.w-base*.036,area.h-base*.036,base*.032,'#ffffffb0',base*.0025);}
 if(isVHS){osdBox(c,m*.4,base*.04,base*.28,base*.05);osdBox(c,w-m*.4-base*.3,base*.04,base*.3,base*.05);const blink=Math.sin(t/140)>0;if(blink){c.fillStyle=accent;c.beginPath();c.arc(m+base*.012,base*.062,base*.011,0,Math.PI*2);c.fill();}text(c,'REC',m+base*.03,base*.068,base*.022,'#eef1f4',600);batteryBadge(c,w-m-base*.05,base*.052,base*.032,base*.016,.72,'#eef1f4');const secs=t/1000,tc=`${String(Math.floor(secs/60)).padStart(2,'0')}:${String(Math.floor(secs%60)).padStart(2,'0')}:${String(Math.floor((secs*30)%30)).padStart(2,'0')}`;text(c,tc,w-m-base*.06,base*.068,base*.02,'#f5c94d',500,'right');}
 else if(isParallax){pillBadge(c,m,base*.048,base*.15,base*.036,'rgba(0,0,0,.4)');c.save();c.fillStyle='#ffe14d';c.beginPath();c.arc(m+base*.026,base*.048+base*.018,base*.009,0,Math.PI*2);c.fill();c.restore();text(c,'LIVE',m+base*.045,base*.048+base*.026,base*.019,'#fff',700);pillBadge(c,w-m-base*.12,base*.048,base*.12,base*.036,'rgba(0,0,0,.4)');text(c,'×'+(1+zEase*.08).toFixed(1),w-m-base*.06,base*.048+base*.026,base*.019,accent,600,'center');}
 else if(isLightstick){osdBox(c,m*.4,base*.04,base*.36,base*.05);osdBox(c,w-m*.4-base*.2,base*.04,base*.2,base*.05);const blink=Math.sin(t/220)>0;if(blink){c.fillStyle='#ff4d6d';c.beginPath();c.arc(m+base*.012,base*.062,base*.009,0,Math.PI*2);c.fill();}text(c,'직캠 · FANCAM',m+base*.028,base*.068,base*.02,'#eef1f4',500);text(c,'BOKEH',w-m-base*.03,base*.068,base*.018,accent,500,'right');}
 else if(isPro){text(c,'MANUAL / 50 MM',m,base*.068,base*.022,'#eef1f4',500);histogram(c,w-m-base*.16,base*.044,base*.16,base*.032,'#ffffffd0');}
 else if(isGrid){text(c,'FRAME · 3 × 3',m,base*.068,base*.022,'#eef1f4',500);pillBadge(c,w-m-base*.1,base*.046,base*.1,base*.032,'rgba(0,0,0,.35)');text(c,'4:3',w-m-base*.05,base*.069,base*.016,'#3ddc84',600,'center');}
 else{c.save();c.fillStyle='#eef1f4';c.beginPath();c.moveTo(m+base*.016,base*.05);c.lineTo(m+base*.004,base*.066);c.lineTo(m+base*.013,base*.066);c.lineTo(m+base*.003,base*.084);c.lineTo(m+base*.026,base*.062);c.lineTo(m+base*.015,base*.062);c.closePath();c.fill();c.restore();text(c,'PHOTO',m+base*.036,base*.068,base*.022,'#eef1f4',500);batteryBadge(c,w-m-base*.05,base*.052,base*.032,base*.016,.94,'#eef1f4');}
 ...
 if(isVHS){osdBox(c,m*.4,area.y+area.h-base*.085,base*.3,base*.065);text(c,'SP · STEREO',m+base*.03,area.y+area.h-base*.06,base*.02,'#e9edef',500);text(c,"'26.09.26",m+base*.03,area.y+area.h-base*.035,base*.019,'#f5c94d',500);}
 if(isLightstick){osdBox(c,w-m-base*.19,area.y+area.h-base*.058,base*.19,base*.034);text(c,'4K · 60P',w-m-base*.095,area.y+area.h-base*.035,base*.016,'#eef1f4c0',600,'center');}
 ...
 if(kind==='clean')modeRow(c,w/2,h-base*.135,['비디오','사진','인물사진'],1,'#ffd60a',base*.016);
 if(isGrid)modeRow(c,w/2,h-base*.135,['동영상','사진','인물사진'],1,'#3ddc84',base*.016);
...
function draw(c,w,h,time=3200){if(state.mode==='music')return music(c,w,h);if(state.mode==='lifecut')return lifecut(c,w,h);focus(c,w,h,time);}
...
function updateButtons(){const has=hasPhoto();for(const id of ['png','gif','replay','playPause','timeline','eyedropper'])$(id).disabled=!has||state.busy;$('clear').disabled=!state.image||state.busy;$('photo').disabled=state.busy;$('choose').disabled=state.busy;$('lifecutClear').disabled=!state.lifecutImages.some(Boolean)||state.busy;document.querySelectorAll('.slot-pick').forEach(b=>b.disabled=!state.lifecutImages[Number(b.dataset.slot)]||state.busy);}
function syncEmptyState(){const has=hasPhoto();$('empty').hidden=has;canvas.hidden=!has;}
function setMode(mode){if(state.busy)return;state.mode=mode;stop();setPicking(false);document.querySelectorAll('[data-mode]').forEach(b=>b.setAttribute('aria-selected',String(b.dataset.mode===mode)));$('musicPanel').hidden=mode!=='music';$('focusPanel').hidden=mode!=='focus';$('lifecutPanel').hidden=mode!=='lifecut';$('sharedPhotoSection').hidden=mode==='lifecut';$('gif').hidden=mode!=='focus';$('replay').hidden=mode!=='focus';$('motionControls').hidden=mode!=='focus';updateButtons();syncEmptyState();refresh(true);}
...
function lifecutSlotUI(index){const label=document.querySelector(`.lifecutInput[data-index="${index}"]`).closest('label');label.classList.toggle('filled',!!state.lifecutImages[index]);const pick=document.querySelector(`.slot-pick[data-slot="${index}"]`);pick.disabled=!state.lifecutImages[index];}
function lifecutAdjustUI(){const t=state.lifecutTransforms[state.lifecutSelected];$('lifecutZoom').value=t.zoom;$('lifecutPanX').value=t.panX;$('lifecutPanY').value=t.panY;document.querySelectorAll('.slot-pick').forEach(b=>b.setAttribute('aria-selected',String(Number(b.dataset.slot)===state.lifecutSelected)));}
...
function setBusy(busy){state.busy=busy;document.querySelectorAll('.editor input,.editor select,.editor textarea,.tabs button,.editor .text-button,.swatch,.slot-pick').forEach(el=>el.disabled=busy);updateButtons();if(busy){resumePreview=previewPlaying;stop();}else if(state.mode==='focus'){refresh();if(resumePreview)play();resumePreview=false;}}
...
for(const id of ['title','artist','elapsed','duration','background','showPalette','size','zoom','panX','panY','caption','captionColor','italic','camera','orientation','loop','lifecutFrame','lifecutDate','lifecutCaption'])$(id).addEventListener('input',()=>refresh(['camera','orientation','loop'].includes(id)));
$('white').onclick=()=>{$('background').value='#ffffff';render();};$('lifecutWhite').onclick=()=>{$('lifecutFrame').value='#ffffff';render();};$('eyedropper').onclick=()=>setPicking(!state.picking);$('replay').onclick=()=>replay(true);$('png').onclick=exportPNG;$('gif').onclick=exportGIF;$('cancel').onclick=()=>state.cancel?.();
...
document.querySelectorAll('.slot-pick').forEach(b=>b.onclick=()=>{state.lifecutSelected=Number(b.dataset.slot);lifecutAdjustUI();});
for(const id of ['lifecutZoom','lifecutPanX','lifecutPanY'])$(id).addEventListener('input',()=>{const t=state.lifecutTransforms[state.lifecutSelected];t.zoom=number('lifecutZoom');t.panX=number('lifecutPanX');t.panY=number('lifecutPanY');refresh();});
$('lifecutClear').onclick=()=>{if(state.busy)return;state.lifecutTokens=state.lifecutTokens.map(t=>t+1);state.lifecutImages=[null,null,null,null];state.lifecutTransforms=[{zoom:1,panX:0,panY:0},{zoom:1,panX:0,panY:0},{zoom:1,panX:0,panY:0},{zoom:1,panX:0,panY:0}];state.lifecutSelected=0;document.querySelectorAll('.lifecutInput').forEach(inp=>{inp.value='';inp.closest('label').classList.remove('filled');});document.querySelectorAll('.slot-pick').forEach(b=>b.disabled=true);lifecutAdjustUI();syncEmptyState();updateButtons();refresh(true);status('사진을 선택하면 저장할 수 있습니다.');};
